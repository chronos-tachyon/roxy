	...
	testData := []testRow{
		{
			...
			RoxyOut: Target{
				Scheme:     constants.SchemeATC,
				Endpoint:   "service",
				Query:      url.Values{"balancer": []string{"random"}, "unique": []string{"foo"}},
				ServerName: "",
				HasSlash:   true,
			},
			Output: "atc:///service?balancer=random&unique=foo",
		},
		{
			Input: "atc:///service/42?unique=foo&location=aws:us-east-1&cps=50",
			RoxyIn: Target{
				Scheme:     constants.SchemeATC,
				Endpoint:   "service/42",
				Query:      url.Values{"unique": []string{"foo"}, "location": []string{"aws:us-east-1"}, "cps": []string{"50"}},
				ServerName: "",
				HasSlash:   true,
			},
			ATC: ATCTarget{
